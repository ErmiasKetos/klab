...
st.title("🌱 KELAP Growth & Resilience Simulator")
st.markdown("""
...
def compute_test_metrics(test_data, volumes, comp_reporting):
    """Computes total test revenue and required testing hours."""
    total_rev = 0.0
    total_hours = 0.0
    for test, config in test_data.items():
        ...
        if comp_reporting:
            price *= 1.15
        total_rev += price * vol
    return total_rev, total_hours

def compute_premium_revenue(reg_consult, onsite_miles):
    """Computes revenue from premium services."""
...
def scenario_results(base_test_rev, premium_rev, fixed_cost, cost_mult, cap_factor, price_mult):
    """Computes scenario-adjusted revenue and profit."""
...
with st.sidebar.expander("Staffing & Capacity"):
    staff_weekly_hours = st.number_input("Total Staff Hours/Week", min_value=0, value=100, step=1)
...
with st.sidebar.expander("Test Pricing & Turnaround"):
    st.markdown("Define tests with base prices and turnaround time multipliers.")
...
# -----------------------------
# Compute base test revenue and required hours
base_test_revenue, total_required_hours = compute_test_metrics(test_data, monthly_volumes, comp_reporting)
...
col1.metric("Required Hours", f"{int(total_required_hours)} hrs")
...
st.markdown("""
The following scenarios adjust capacity, pricing, and costs:
...
for name, params in scenario_params.items():
    ...
    scenarios.append({"Scenario": name, "Adjusted Revenue ($)": round(rev,2), "Adjusted Profit ($)": round(profit,2)})
...
st.markdown("""
Simulating uncertainty in realized capacity and pricing for the final month.
""")
sim_revenues = []
sim_profits = []
...
st.markdown("""
**Insights:**  
- The simulation shows the range of possible revenue and profit outcomes.  
- Adjust the uncertainty parameters to evaluate risk and resilience.
""")
